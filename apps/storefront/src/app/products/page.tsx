...
import { Search, SlidersHorizontal, X, Package, ChevronLeft, ChevronRight, Grid, List } from 'lucide-react';
...
function ProductsContent() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setPage(1);
    updateUrl({ search: searchInput, categoryId, type, sortBy, sortOrder, limit });
  };
  ...
  const handleFilterChange = (key: string, value: string) => {
    ...
    if (key === 'categoryId') {
      setCategoryId(value);
      updateUrl({ search, categoryId: value === 'all' ? undefined : value, type: type === 'all' ? undefined : type, sortBy, sortOrder, limit });
    } else if (key === 'type') {
      setType(value as 'PHYSICAL' | 'DIGITAL' | 'all');
      updateUrl({ search, categoryId: categoryId === 'all' ? undefined : categoryId, type: value === 'all' ? undefined : value, sortBy, sortOrder, limit });
    } else if (key === 'sortBy') {
      setSortBy(value);
      updateUrl({ search, categoryId, type, sortBy: value, sortOrder, limit });
    } else if (key === 'sortOrder') {
      setSortOrder(value as 'asc' | 'desc');
      updateUrl({ search, categoryId, type, sortBy, sortOrder: value, limit });
    } else if (key === 'limit') {
      setLimit(parseInt(value));
      updateUrl({ search, categoryId, type, sortBy, sortOrder, limit: parseInt(value) });
    }
  };
  ...
  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    updateUrl({ page: newPage, search, categoryId, type, sortBy, sortOrder, limit });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const activeFiltersCount = [search, categoryId !== 'all' ? categoryId : null, type !== 'all' ? type : null].filter(Boolean).length;

  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <section className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white py-8 md:py-12">
              ...
              <SheetContent side="left" className="w-80">
                ...
                <div className="space-y-6 mt-6">
                  ...
                  <div>
                    ...
                    <Select value={String(limit)} onValueChange={(v) => handleFilterChange('limit', v)}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
